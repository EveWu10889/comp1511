...
int compare (struct date *when, struct date *start_day, struct date *finish_day);
int count (struct date *time);
...
void between_days_whales(struct pod *first_pod, struct date *start_day, struct date *finish_day) {
    ...
    while (n != NULL) {
        if(compare(n->when, start_day, finish_day)) {
...
int compare (struct date *when, struct date *start_day, struct date *finish_day) {
    int num1 = count(when);
    int num2 = count(start_day);
    int num3 = count(finish_day);
...
/*int compare (struct date *when, struct date *start_day, struct date *finish_day) {
    if (compare_year(when, start_day, finish_day) == 1) {
        return 1;   
    } 
    else if (compare_year(when, start_day, finish_day) == -1) {
        if ((when->year == start_day->year) && (when->year == finish_day->year)){
            if (((when->month > start_day->month) && (when->month < finish_day->month)) || ((when->month = start_day->month) && (when->day >= start_day->day)) || ((when->month = finish_day->month) && (when->day <= finish_day->day) )) {
            return 1;
            }
            else
            {
        ...
        else if (when->year == start_day->year) {
            if ((when->month > start_day->month) || ((when->month == start_day->month) && (when->day >= start_day->day) )) {
                return 1;            
            } 
            else {
                return 0;
        ...
        else if (when->year == finish_day->year){            
            if ((when->month < finish_day->month) || ((when->month == start_day->month) && (when->day <= start_day->day) )) {
                return 1;            
            } 
            else {
                return 0;
